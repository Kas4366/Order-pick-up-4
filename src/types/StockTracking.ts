...
import { findImageFileWithFallback } from '../utils/imageUtils';
...
export const useOrderData = () => {
  ...
  const handleMarkForReorder = (order: Order) => {
    ...
    const newItem: StockTrackingItem = {
      ...
      weight: order.weight,
      newLocation: order.newLocation,
      localImageSource: order._isLocalImage ? {
        sku: order._originalSkuForLocalImage || order.sku,
        folderName: csvImagesFolderInfo?.folderName || 'Unknown'
      } : undefined,
    };
        ...
        if (isArchiveInitialized) {
          try {
            ...
            if (archiveResult.foundInArchive && archiveResult.orders.length > 0) {
              ...
              // Handle archived order loading with image restoration
              await handleLoadArchivedOrder(archivedOrder);
  ...
  // Handle loading archived order with image restoration
  const handleLoadArchivedOrder = async (archivedOrder: ArchivedOrder) => {
    let imageUrl = archivedOrder.imageUrl;
    let isLocalImage = false;
    let originalSkuForLocalImage = archivedOrder.sku;

    if (archivedOrder.localImageSource) {
      console.log('🖼️ Archived order has local image source info, attempting to re-generate image URL...');
      try {
        const folderHandle = await fileHandlePersistenceService.getHandle('csvImagesFolder'); // Assuming CSV images folder is the source
        if (folderHandle) {
          const hasPermission = await fileHandlePersistenceService.validateAndRequestPermission(folderHandle);
          if (hasPermission) {
            const reGeneratedUrl = await findImageFileWithFallback(
              folderHandle,
              archivedOrder.localImageSource.sku, // Use the SKU stored in localImageSource
              archivedOrder.localImageSource.sku // Pass SKU again for fallback logic
            );
            if (reGeneratedUrl) {
              imageUrl = reGeneratedUrl;
              isLocalImage = true;
              originalSkuForLocalImage = archivedOrder.localImageSource.sku;
              console.log('✅ Successfully re-generated local image URL for archived order.');
            } else {
              console.warn('⚠️ Could not re-generate local image URL for archived order.');
            }
          } else {
            console.warn('⚠️ Permission denied for local images folder, cannot re-generate image URL.');
          }
        } else {
          console.warn('⚠️ No folder handle found for local images, cannot re-generate image URL.');
        }
      } catch (error) {
        console.error('❌ Error re-generating local image URL for archived order:', error);
      }
    }
    ...
    const orderFromArchive: Order = {
      ...
      _originalSkuForLocalImage: originalSkuForLocalImage,
